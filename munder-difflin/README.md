...
<img src="./docs/logo.png" alt="The Watchdogs — security research multi-agent harness" width="180">

# The Watchdogs

### Multi-agent security research harness with hard human approval gates
...
**Free, open source, and secure** — an authorized security research harness built on top of Munder Difflin. It repurposes the hive, router, PTY layer, and approval queue for coordinated offensive security operations: reconnaissance, vulnerability discovery, exploitation validation, and reporting across dedicated agent desks.
...
> - **/jonathan** → [HackerGPT](https://github.com/stalane/HackerGPT) (GPL-3.0)
...
> [!NOTE]
> **The Watchdogs Security Architecture**  
> The office operates through a disciplined pentest lifecycle:  
> **Recon → Vulnerability Discovery → Exploitation → Reporting**  
> Coordinated by **Michael** (Manager) with specialized desks:  
> - **/chris** (Strix): Exploitation validation — takes candidate findings from discovery and produces real, working PoCs before anything is marked confirmed  
> - **/daniel** (Shannon): Analytical code-aware recon and deeper analysis  
> - **/jonathan** (HackerGPT): Reconnaissance & intelligence gathering — maps attack surface, discovers live ports/services/endpoints, and feeds findings into discovery  
> - **/david** (HackBot): Practical automation, fast CVE lookups, and payload suggestions  
> - **/wrench** / **/gabriel** (HexStrike AI): Tool floor & 150+ tool MCP backbone  
> - **/maverick** (Skills Service): On-demand librarian for 818 offensive security skills  
> - **/rooster** (AI Red Team): Non-destructive testing for AI/chatbot targets with pre-execution gating  
>
> **Hard Human Approval Gate:** Any exploitation-class or intrusive action (`requires_approval: true`) strictly pauses in the approval queue until an operator explicitly approves it in the UI. No autonomous exploitation without human sign-off.
