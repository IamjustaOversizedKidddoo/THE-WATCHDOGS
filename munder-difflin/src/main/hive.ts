...
import { parseDirectCallsign, routePentestFinding } from './watchdogsRouter';
import { MaverickSkillsService, SkillProposal } from './maverick';
import { JonathanContinuousReconService } from './jonathanContinuousRecon';
import { ProgramRegistryService } from './programRegistry';
import { DuplicateCheckerService } from './duplicateChecker';
import { ReportQualityService, QualityReportDraft } from './reportQualityService';
import { MichaelSchedulerService, ScheduledTask } from './michaelScheduler';
import { RoosterService, type RoosterSessionOptions, type RoosterSessionResult, type RoosterPendingApproval } from './roosterService';
...
export class HiveManager {
   ...
   */
  public readonly approvalQueue: ApprovalQueue;
  public readonly maverickService: MaverickSkillsService;
  public readonly jonathanReconService: JonathanContinuousReconService;
  public readonly programRegistry: ProgramRegistryService;
  public readonly duplicateChecker: DuplicateCheckerService;
  public readonly reportQualityService: ReportQualityService;
  public readonly michaelScheduler: MichaelSchedulerService;
  public readonly roosterService: RoosterService;
  ...
  constructor(
    ...
    michaelScheduler?: MichaelSchedulerService,
    roosterService?: RoosterService
  ) {
    this.approvalQueue = new ApprovalQueue(this.getHome);
    this.maverickService = new MaverickSkillsService();
    this.programRegistry = programRegistry || new ProgramRegistryService();
    this.duplicateChecker = duplicateChecker || new DuplicateCheckerService();
    this.reportQualityService = reportQualityService || new ReportQualityService();
    this.michaelScheduler = michaelScheduler || new MichaelSchedulerService(this.programRegistry);
    this.roosterService = roosterService || new RoosterService(this.programRegistry, this.approvalQueue);
    this.jonathanReconService = new JonathanContinuousReconService(
      () => this.root(),
      (msg) => {
        this.send(msg, 'jonathan');
      },
  ...
  /** Approve a pending message held in the Phase 2.5 approval queue */
  approveMessage(id: string): boolean {
    const pending = this.approvalQueue.getPending(id);
    if (!pending) return false;

    // Addition 2 & 6: Program Registry Scope, Staleness & Program ID Cross-Check Gate
    const target = pending.message.finding?.target || (pending.message as any).target;
    const programId = pending.message.program_id || pending.message.finding?.program_id;
    if (target) {
      const verification = this.programRegistry.verifyTargetForAction(
        target,
        new Date(),
        undefined,
        programId
    ...
    const msg = this.approvalQueue.approve(id);
    if (!msg) return false;
    this.deliver(msg, msg.to);
    this.appendLog({ kind: 'approval-granted', id: msg.id, to: msg.to });
    this.commit(`hive: approved msg ${msg.id} -> ${msg.to}`);
    return true;
  }

  /** Reject a pending message held in the Phase 2.5 approval queue */
  rejectMessage(id: string, reason?: string): boolean {
    const success = this.approvalQueue.reject(id, reason);
    if (success) {
      this.appendLog({ kind: 'approval-denied', id, reason });
      this.commit(`hive: rejected msg ${id}`);
    }
    return success;
  }

  /**
   * Addition 3: Pre-submission duplicate check immediately before submission.
   * Always executes fresh (never returns cached advisory result).
   */
  async checkDuplicatePreSubmission(
    finding: WatchdogsFinding,
    platformOverride?: string
  ) {
    const prog = platformOverride
      ? { platform: platformOverride }
      : this.programRegistry.findProgramForTarget(finding.target) || { platform: 'hackerone' };
    return this.duplicateChecker.checkPreSubmission(finding, prog as any);
  }

  /**
   * Addition 4: Report Quality Pass Before Submission.
   * Generates an evidence-traced, redacted, CVSS-reasoned draft in PENDING_HUMAN_APPROVAL state.
   */
  generateQualityReport(
    finding: WatchdogsFinding,
    vulnerabilityTitle?: string
  ): QualityReportDraft {
    const disclosed = this.duplicateChecker.getRecentDisclosedReports();
    return this.reportQualityService.generateReportDraft(finding, {
      disclosedReports: disclosed,
      vulnerabilityTitle
    });
  }

  /**
   * Addition 4: Human sign-off on quality report draft.
   */
  approveQualityReport(reportId: string, approver = 'human'): QualityReportDraft {
    return this.reportQualityService.approveDraft(reportId, approver);
  }

  /**
   * Addition 4: Enforce human sign-off before submission.
   */
  submitQualityReport(reportId: string): { submitted: boolean; reason?: string } {
    return this.reportQualityService.submitReport(reportId);
  }

  /**
   * Addition 5: Triggers Maverick's periodic review of recently disclosed reports
   * across active programs via official API (zero scraping).
   */
  async reviewDisclosedSkills(): Promise<SkillProposal[]> {
    const programs = this.programRegistry.listPrograms();
    return this.maverickService.reviewDisclosedReports(programs, this.duplicateChecker);
  }

  /**
   * Addition 5: Returns all skill proposals awaiting human sign-off.
   */
  getPendingSkillProposals(): SkillProposal[] {
    return this.maverickService.getPendingProposals();
  }

  /**
   * Addition 5: Returns a specific skill proposal by ID.
   */
  getSkillProposal(proposalId: string): SkillProposal | null {
    return this.maverickService.getProposal(proposalId);
  }

  getProposal(proposalId: string): SkillProposal | null {
    return this.maverickService.getProposal(proposalId);
  }

  /**
   * Addition 5: Human approval gate for proposed skill addition / swap.
   */
  approveSkillProposal(proposalId: string, approver = 'human') {
    return this.maverickService.approveProposal(proposalId, approver);
  }

  /**
   * Addition 5: Human operator rejection of proposed skill addition / swap.
   */
  rejectSkillProposal(proposalId: string, reason: string, reviewer = 'human') {
    return this.maverickService.rejectProposal(proposalId, reason, reviewer);
  }

  /**
   * Addition 6: Schedules a task under /michael parallel-program mode.
   * Strictly enforces mandatory program_id, per-program testing restrictions,
   * parallel recon (/jonathan), and single-concurrency validation queues (/chris, /daniel, /david).
   */
  scheduleMichaelTask(
    taskInput: {
      id?: string;
    ...
    options?: { currentTime?: Date; customPacingMs?: number }
  ) {
    return this.michaelScheduler.scheduleTask(taskInput as any, options);
  }

  completeMichaelTask(taskId: string, completedTime = new Date()) {
    return this.michaelScheduler.completeTask(taskId, completedTime);
  }

  getMichaelTask(taskId: string): ScheduledTask | null {
    return this.michaelScheduler.getTask(taskId);
  }

  getActiveTaskForDesk(desk: string): ScheduledTask | null {
    return this.michaelScheduler.getActiveTaskForDesk(desk);
  }

  getDeskQueue(desk: string): ScheduledTask[] {
    return this.michaelScheduler.getQueueForDesk(desk);
  }

  // ── Addition 7: /rooster public hive methods ────────────────────────────────

  /**
   * Addition 7: Run a /rooster AI/chatbot test session through the hive.
   *
   * Enforces the full gate hierarchy — scope, live-tool authorization, and
   * pre-execution approval — before any probe is issued. Findings that pass all
   * gates and involve live tool invocation are handed to the existing approval
   * queue (the same queue used by /chris, /daniel, /david) so they are visible
   * to the human operator through the same UI path.
   */
  runRoosterSession(options: RoosterSessionOptions): RoosterSessionResult {
    const result = this.roosterService.runSession(options);

    // If the session produced a finding that requires post-execution approval
    // (live tool-invocation was used and succeeded), route it through the
    // standard approval queue so it surfaces alongside other exploitation-class
    // actions — identical handling to /chris producing a requires_approval finding.
    if (result.finding && result.finding.requires_approval) {
      this.send(
        {
          to: 'michael',
          from: 'rooster',
          act: 'inform' as const,
          subject: `[/rooster] ${options.technique} finding on ${options.target}`,
          body: result.finding.evidence,
    ...
    if (result.gate.allowed) {
      this.appendLog({
        kind: 'rooster-session-complete',
        target: options.target,
        program_id: options.program_id,
        technique: options.technique,
        outcome: result.session?.outcome,
        non_destructive_mode: result.session?.non_destructive_mode
      });
    } else {
      this.appendLog({
        kind: 'rooster-session-blocked',
        target: options.target,
        program_id: options.program_id,
        technique: options.technique,
  ...
  /**
   * Addition 7: Human operator approves a queued /rooster pre-execution approval.
   * Must be called before a live tool-invocation test may run.
   */
  approveRoosterPreExecution(approvalId: string): boolean {
    const ok = this.roosterService.approvePreExecution(approvalId);
    if (ok) {
      this.appendLog({ kind: 'rooster-preexec-approved', approvalId });
      this.commit(`hive: rooster pre-exec approved ${approvalId}`);
    }
    return ok;
  }

  /**
   * Addition 7: Human operator rejects a queued /rooster pre-execution request.
   */
  rejectRoosterPreExecution(approvalId: string, reason?: string): boolean {
    const ok = this.roosterService.rejectPreExecution(approvalId, reason);
    if (ok) {
      this.appendLog({ kind: 'rooster-preexec-rejected', approvalId, reason });
      this.commit(`hive: rooster pre-exec rejected ${approvalId}`);
    }
    return ok;
  }

  /**
   * Addition 7: Returns all /rooster pre-execution approval requests currently
   * awaiting operator decision.
   */
  listPendingRoosterApprovals(): RoosterPendingApproval[] {
    return this.roosterService.listPendingApprovals();
  }

  /**
   * Addition 7: Returns a specific /rooster pre-execution approval record by ID.
   */
  getRoosterPendingApproval(approvalId: string): RoosterPendingApproval | null {
    return this.roosterService.getPendingApproval(approvalId);
  }


  private routeMessage(msg: HiveMessage): void {
    // Phase 2.4b: Direct callsign addressing (e.g. /gabriel, /chris, /daniel, /jonathan, /david, /michael)
    const callsign = parseDirectCallsign(msg.body || msg.subject);
    if (callsign && (msg.from === 'human' || msg.from === 'system' || msg.from === 'operator')) {
      msg.to = callsign.targetDesk;
      if (callsign.cleanedText) {
        msg.body = callsign.cleanedText;
      }
    }

    // Phase 2.4: Pentest state machine routing
    if (msg.finding) {
      const decision = routePentestFinding(msg.finding);
      if (decision.blocked) {
        this.appendLog({ kind: 'pentest-blocked', reason: decision.reason, id: msg.id });
      }
      if (decision.requiresApproval) {
        msg.finding.requires_approval = true;
      }
      if (decision.nextDesk && (msg.to === 'god' || msg.to === 'michael')) {
        msg.to = decision.nextDesk;
      }

      // Addition 3: Advisory duplicate check upon entering reporting phase
      if (msg.finding.phase === 'reporting') {
        const prog = this.programRegistry.findProgramForTarget(msg.finding.target) || { platform: 'hackerone' };
        this.duplicateChecker.checkOnReportingEntry(msg.finding, prog).then((res) => {
          if (res.status === 'FLAGGED_POSSIBLE_DUPLICATE') {
            this.appendLog({
              kind: 'duplicate-check-flagged',
              id: msg.id,
              target: msg.finding!.target,
    ...
    // Phase 2.5: Hardened Approval Gate
    // Any exploitation-class or requires_approval action STOPS here and cannot dispatch.
    const requiresApproval = msg.finding?.requires_approval === true || (msg as any).requires_approval === true;
    if (requiresApproval && !this.approvalQueue.isPending(msg.id)) {
      this.approvalQueue.enqueue(msg);
      this.appendLog({ kind: 'approval-required', from: msg.from, to: msg.to, id: msg.id, subject: msg.subject });
      return; // HARD STOP: never reaches inbox without human approval!
    }

    // Maverick Setup: Skill Guidance Service
    // When a desk queries /maverick for guidance, Maverick checks the desk's manifest and responds.
    if (msg.to === 'maverick') {
      const requestingDesk = msg.from;
      const query = msg.body || msg.subject;
      const result = this.maverickService.resolveSkillForDesk(requestingDesk, query);

      if (result.allowed && result.content) {
        // Return matched skill content to requesting desk's inbox
        this.deliver({
          id: `maverick-reply-${msg.id}`,
          conversation: msg.conversation,
          in_reply_to: msg.id,
          from: 'maverick',
          to: requestingDesk,
          act: 'inform',
          subject: `Skill Guidance: ${result.skillName}`,
          body: result.content,
          hops: 0,
        ...
        // Flag to requester (and human if flaggedToHuman)
        this.deliver({
          id: `maverick-reply-${msg.id}`,
          conversation: msg.conversation,
          in_reply_to: msg.id,
          ...
          to: requestingDesk,
          act: 'refuse',
          subject: `Skill Guidance: Request Denied / Flagged`,
          body: result.reason || 'Skill request not permitted.',
          hops: 0,
          requires_reply: false,
          needs_human: result.flaggedToHuman === true,
          created_at: new Date().toISOString()
        }, requestingDesk);
        if (result.flaggedToHuman) {
          this.appendLog({
            kind: 'maverick-flagged',
            from: requestingDesk,
            query,
  ...
  stopRouter(): void {
    ...
    this.jonathanReconService.stopScheduler();
  }
